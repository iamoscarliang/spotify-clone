...
public class ArtistResultFragmentTest {
    ...
    public void setUp() {
        ...
        viewModel = mock(ArtistResultViewModel.class);
        when(viewModel.getResult()).thenReturn(result);
        when(viewModel.getLoadMoreState()).thenReturn(loadMoreState);

        FragmentScenario<ArtistResultFragment> scenario = FragmentScenario.launchInContainer(
                ArtistResultFragment.class,
                null,
                new FragmentFactory() {
                    @NonNull
                    @Override
    ...
    public void testSuccess() {
        result.onNext(Result.success(createArtists(2, UNKNOWN_ID, "foo")));
    ...
    public void testLoadNextPage() {
        result.onNext(Result.success(createArtists(20, UNKNOWN_ID, "foo")));
    ...
    public void testNavigate() {
        result.onNext(Result.success(singletonList(createArtist("foo", UNKNOWN_TITLE))));

        onView(listMatcher().atPosition(0)).perform(longClick());
        verify(navController).navigate(Uri.parse("android-app://artistFragment/foo"));
    }
