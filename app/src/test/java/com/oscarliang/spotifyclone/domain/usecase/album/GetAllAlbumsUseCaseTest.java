...
public class GetAllAlbumsUseCaseTest {

    private AlbumRepository mRepository;
    private GetAllAlbumsUseCase mUseCase;

    @Before
    public void init() {
        mRepository = mock(AlbumRepository.class);
        mUseCase = new GetAllAlbumsUseCase(mRepository);
    }

    @Test
    public void execute() {
        mUseCase.execute(10);
        verify(mRepository).getAllAlbums(10);
    }
